...
int8_t postamble() {
    uint16_t reg_addr[5] = {0x0514, 0x001C, 0x0540, 0x0B24, 0x0B25};
    uint8_t  data[5]     = {0x01, 0x01, 0x00, 0xC3, 0x02};
    for (int i = 0; i < 5; i++) {
        if (i2c_write(reg_addr[i], data[i]) < 0) { return -1; }
    }
    return 0;
...
//     while (i < SI5345_REVD_REG_CONFIG_NUM_REGS-5){
//         reg_addr = si5345_revd_registers[i].address;
//         data     = si5345_revd_registers[i].value;
//         preamble();
//         ret_w = i2c_write(reg_addr, data);
//         postamble();
//         for (int j = 3; j < SI5345_REVD_REG_CONFIG_NUM_REGS - 5; j++) {
//             reg_addr     = si5345_revd_registers[j].address;
//             correct_data = si5345_revd_registers[j].value;
//             ret_r = i2c_read(reg_addr, &data);
//             if ((data != correct_data) || ret_w < 0 || ret_r < 0) break;
//             i++;
//         }
//     }
//     return 0;
...
int8_t skip_13() {
    ...
    int      ret;
    // while (i < SI5345_REVD_REG_CONFIG_NUM_REGS - 5) {
    for (i = 3; i < SI5345_REVD_REG_CONFIG_NUM_REGS - 5; i++) {
        system("source ./i2cdump.sh");
        preamble();
        if (i == 13) continue;
        if (i % 10 == 0) printf("DO NOT STOP THIS PROCESS BY CTRL-C!\n");
...
int main(int argc, char *argv[]) {
    ...
    struct option longopts[] = {
        ...
        {  "postamble",       no_argument, NULL, 'P'},
        {    "skip-13",       no_argument, NULL, 'b'},
        {       "help",       no_argument, NULL, 'h'},
        {            0,                 0,    0,   0}  // indicates the termination of this array
    };
    char optstring[] = "a:d:rRwWspPbh";
    int  opt;
    int  longindex;
    ...
    while ((opt = getopt_long(argc, argv, optstring, longopts, &longindex)) !=
        ...
        switch (opt) {
            ...
            case 'P': postamble(); return 0;
            case 'b': skip_13(); return 0;
